...
function ShowOne_v002(i){
		var ch = "";
			...
			ch += '</div>';
			ch += '<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="overflow:hidden;background-color:green;height: 100%;padding-top:50px;">';
			ch += '&nbsp;<h4><b>'+ListSchool[i].name+'</b></h4>';
...
function ShowOne_v003(newlist,i){
		var ch = "";
			ch += '<div style="background-color:gray;color:white;height:250px;margin:0px;padding:0px;margin-bottom:10px;" class="col-lg-12 col-md-12 col-sm-12 col-xs-12">';
			ch += '<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="background-color:silver;height:100%;margin:0px;padding:0px;">';
			ch += '<img src="img/'+newlist[i].FirstImg()+'.jpg" height="100%" width="100%" alt=""/>';
			ch += '</div>';
			ch += '<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="background-color:green;height: 100%;padding-top:50px;">';
			ch += '&nbsp;<h4><b>'+newlist[i].name+'</b></h4>';
			ch += newlist[i].Description;
			ch += '</div>';
			ch += '</div>';
		return ch;
}
function ShowAll_v003(newlist){
	var ch = "";//alert(newlist.length);
	for (var i = 0; i < newlist.length; i++) {
		ch += ShowOne_v003(newlist,i)+"\n";
	}
	return ch;
}

function Search(cityName,type){
	var newlist = [];
	var ch = "";//alert('start');
	for (var i = 0; i < ListSchool.length; i++) {
		if (ListSchool[i].City == cityName && ListSchool[i].Type == type) {
			ch += ListSchool[i].Show();
			newlist.push(ListSchool[i]);
		}
	}
	if(ch!=""){
	//	alert("ch :::\n\n"+ch);
	//alert(ShowAll_v003(newlist));
	return ShowAll_v003(newlist);
	}
	else{
		return "There is no school by those criteria";
	}
}
...
function Search_v002(cityName,type,keyword){
	var newlist = [];
	var ch = "";//alert('start kw: '+keyword);
	for (var i = 0; i < ListSchool.length; i++) {
		if (ListSchool[i].City == cityName && ListSchool[i].Type == type) {
			if((ListSchool[i].name.toUpperCase().indexOf(keyword.trim().toUpperCase())!=-1)||(ListSchool[i].Description.toUpperCase().indexOf(keyword.trim().toUpperCase())!=-1))
			{
				ch += ListSchool[i].Show();
				newlist.push(ListSchool[i]);
			}
	...
	return ShowAll_v003(newlist);
		...
		return "<h2>There is no school by those criteria</h2>";
	}
}
